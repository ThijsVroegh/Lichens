...
# Shared color palette aligned with red list figures
# Keys mirror red list categories for reuse across other plots
PALETTE = {
    'not_threatened': '#abd9e9',
    'sensitive': '#fdae61',
    'vulnerable': '#f46d43',
    'threatened': '#d73027',
    'seriously_threatened': '#a50026',
    'disappeared': '#313695'
}

# Default ordered colors to reuse for categorical sequences
DEFAULT_COLORS = [
    PALETTE['not_threatened'],
    PALETTE['sensitive'],
    PALETTE['vulnerable'],
    PALETTE['threatened'],
    PALETTE['seriously_threatened'],
    PALETTE['disappeared']
]

# Map our substrate main categories to the palette for consistent bar colors
CATEGORY_COLORS = {
    'Bark': PALETTE['sensitive'],
    'Stone': PALETTE['disappeared'],
    'Wood': PALETTE['not_threatened'],
    'Soil': PALETTE['vulnerable'],
    'Other': PALETTE['seriously_threatened'],
    'Unknown': '#9e9e9e'
}

# Load the data from the Excel files
#data = pd.read_excel('data/data.xlsx')
data = pd.read_excel('data/KmhokkenPerSoort.5.xlsx')

substrats = pd.read_excel('data/substrats.xlsx')

# import os
# print(os.path.isfile('data/data.xlsx'))  # Should print True if the file exists

# amount of rows in data
print(len(data)) # 43804 rows
...
print(len(unique_species)) # 494 unique species

unique_locations = data['LOCATION'].unique()
print(len(unique_locations)) # 60 unique locations


unique_kmhok = data['KMHOK'].unique()
print(len(unique_kmhok)) # 573 unique locations

# Get unique combinations of location and kmhok
location_kmhok_combos = data[['LOCATION', 'KMHOK']].drop_duplicates()
print(f"Number of unique location-KMHOK combinations: {len(location_kmhok_combos)}")

# Save the unique location-KMHOK combinations to a CSV file
output_path = 'data/location_kmhok_combinations.csv'

# Save to CSV with descriptive column names
location_kmhok_combos.to_csv(output_path, index=False)
print(f"\nSaved location-KMHOK combinations to {output_path}")


# Display first few combinations
print("\nSample of location-KMHOK combinations:")
print(location_kmhok_combos.head())

# amount of missing values in KMHOK
missing_kmhok = data['KMHOK'].isna().sum()
print(missing_kmhok) # 0 missing values


# Count occurrences per location
location_counts = location_kmhok_combos['LOCATION'].value_counts()
print("\nNumber of KMHOK squares per location:")
print(location_counts.head())

# Basic statistics
avg_kmhok_per_location = location_kmhok_combos.groupby('LOCATION').size().mean()
print(f"\nAverage number of KMHOK squares per location: {avg_kmhok_per_location:.2f}")


unique_genus = data['GENUS'].unique()
print(len(unique_genus)) # 210 unique genus
...
def plot_substrate_distribution(data):
    ...
    ax = plt.gca()
    bar_colors = [CATEGORY_COLORS.get(cat, PALETTE['not_threatened']) for cat in substrate_counts.index]
    bars = ax.bar(range(len(substrate_counts)), substrate_counts.values, color=bar_colors, edgecolor='white')
    ...
    formatted_legend = []
    for category in substrate_counts.index:
        present_codes = (
            data.loc[data['SubstrateCategory'] == category, 'SUB']
                .dropna()
                .astype(str)
                .unique()
                .tolist()
        ...
        if present_codes:
            present_codes_sorted = sorted(present_codes, key=lambda x: x.lower())
            grouped = [', '.join(present_codes_sorted[i:i+8]) for i in range(0, len(present_codes_sorted), 8)]
...
def plot_species_per_substrate(data):
    ...
    # Create vertical bar plot
    ax = plt.gca()
    bar_colors = [CATEGORY_COLORS.get(cat, PALETTE['not_threatened']) for cat in species_counts.index]
    bars = ax.bar(range(len(species_counts)), species_counts.values, color=bar_colors, edgecolor='white')
    ...
    # Dynamic legend: only abbreviations present in the plotted data per category
    formatted_legend = []
    for category in species_counts.index:
        present_codes = (
            data.loc[data['SubstrateCategory'] == category, 'SUB']
                .dropna()
                .astype(str)
...
def plot_top_substrates(data, substrats):
    """Create horizontal bar plot of top 25 substrates by observations with full substrate names"""
    ...
    obs_bars = ax.barh(y_pos, top_substrates['AMOUNT'], color=PALETTE['not_threatened'], edgecolor='white', label='Number of Observations')
    ...
    def get_full_name(code):
        try:
            name = substrats.loc[substrats['AfkSub'] == code, 'NaamSub'].iloc[0]
            if pd.notna(name):
                return f"{code} ({str(name)})"
            else:
                return str(code)
        except Exception:
            return str(code)
...
def plot_bark_distribution(data, substrats):
    ...
    bars = ax.barh(range(len(bark_counts)), bark_counts.values, color=CATEGORY_COLORS.get('Bark', PALETTE['sensitive']), edgecolor='white')
    ...
    # Save the plot
    plt.savefig('plots/bark_distribution.jpg', dpi=300, bbox_inches='tight')
    plt.close()
...
def plot_wood_distribution(data, substrats):
    ...
    wood_data = wood_data[~wood_data['SUB'].astype(str).str.lower().eq('th')]
    ...
    # Save the plot
    plt.savefig('plots/wood_distribution.jpg', dpi=300, bbox_inches='tight')
    plt.close()
...
def plot_temporal_trends(data):
    ...
    # Remove the last year to avoid incomplete data
    max_year = yearly_data['DATE'].max()
    yearly_data = yearly_data[yearly_data['DATE'] < max_year]
    ...
    # Plot 2: Relative frequencies with 5-year aggregation
    # Calculate species relative frequencies per year
    yearly_species = data.groupby([data['DATE'].dt.year, 'SPECIES'])['AMOUNT'].sum().reset_index()

    # Remove the last year from species data as well
    yearly_species = yearly_species[yearly_species['DATE'] < max_year]

    # Create 5-year periods for smoother trends
    yearly_species['period'] = (yearly_species['DATE'] // 5) * 5
    period_species = yearly_species.groupby(['period', 'SPECIES'])['AMOUNT'].sum().reset_index()

    # Pivot data for trend analysis
    pivot_species = period_species.pivot(index='period', columns='SPECIES', values='AMOUNT').fillna(0)
    ...
    # Function to create abbreviated species names
    def abbreviate_species_name(full_name):
        """Convert full species name to abbreviated format (Genus. species)"""
        if pd.isna(full_name):
            return "Unknown"

        # Handle common lichen species with specific abbreviations
        # Map species names to their abbreviated forms
        species_abbreviations = {
            'tenella': 'P. tenella',
            'parietina': 'X. parietina',
            'punctata': 'A. punctata',
            'adscendens': 'P. adscendens',
            'sulcata': 'P. sulcata',
            'orbicularis': 'P. orbicularis',
            'incana': 'L. incana',
            'reflexa': 'C. reflexa',
            'dispersa': 'M. dispersa',
            'concolor': 'C. concolor'
        ...
        # Check if we have a specific abbreviation for this species
        if str(full_name) in species_abbreviations:
            return species_abbreviations[str(full_name)]
    ...
    # Plot relative frequencies for top species with abbreviated names
    ...
    #        "Bottom: Relative frequencies of the 10 most common species over 5-year periods,\n"
    #        "normalized by total observations to account for sampling effort. Last year excluded.")
    # plt.figtext(0.1, 0.02, note, wrap=True, horizontalalignment='left', fontsize=8)
...
def plot_trend_analysis(data):
    ...
    """
    # Calculate yearly relative frequencies, excluding the last year
    yearly_species = data.groupby([data['DATE'].dt.year, 'SPECIES'])['AMOUNT'].sum().reset_index()

    # Remove the last year to avoid incomplete data
    max_year = yearly_species['DATE'].max()
    yearly_species = yearly_species[yearly_species['DATE'] < max_year]
    ...
    def abbreviate_species_name(full_name):
        ...
        # Handle common lichen species with specific abbreviations
        species_abbreviations = {
            'adglutinata': 'H. adglutinata',
            'barkmaniana': 'L. barkmaniana',
            'concolor': 'C. concolor',
            'eleochroma': 'L. eleochroma',
            'rufescens': 'P. rufescens',
            'muralis': 'P. muralis',
            'conizaeoides': 'L. conizaeoides',
            'citrina': 'F. citrina',
            'dispersa': 'M. dispersa',
            'stigmatea': 'L. stigmatea'
        }
    ...
    for i, species in enumerate(top_increasing.index):
        ...
        abbreviated_name = abbreviate_species_name(species)
        ax1.plot(species_trends.index, data, marker='o', color=color, label=f"{abbreviated_name} ({top_increasing[species]:.2e})")

        # Add trend line
        x = np.arange(len(species_trends.index))
        z = np.polyfit(x, data, 1)
        p = np.poly1d(z)
        ax1.plot(species_trends.index, p(x), '--', alpha=0.5, color=color)
    ...
    for i, species in enumerate(top_decreasing.index):
        ...
        color = DEFAULT_COLORS[i % len(DEFAULT_COLORS)]
        abbreviated_name = abbreviate_species_name(species)
        ax2.plot(species_trends.index, data, marker='o', color=color, label=f"{abbreviated_name} ({top_decreasing[species]:.2e})")

        # Add trend line
        x = np.arange(len(species_trends.index))
        z = np.polyfit(x, data, 1)
        p = np.poly1d(z)
        ax2.plot(species_trends.index, p(x), '--', alpha=0.5, color=color)
...
def main():
    ...
    fig1.savefig('plots/frequency_distribution.jpg', bbox_inches='tight')
    plt.close(fig1)

    # Create and save substrate distribution plot
    fig2 = plot_substrate_distribution(data)
    fig2.savefig('plots/substrate_distribution.jpg', bbox_inches='tight')
    plt.close(fig2)

    # Create and save species per substrate plot
    fig3 = plot_species_per_substrate(data)
    fig3.savefig('plots/species_per_substrate.jpg', bbox_inches='tight')
    plt.close(fig3)

    # Create and save top substrates plot
    fig4 = plot_top_substrates(data, substrats)
    fig4.savefig('plots/top_substrates.jpg', bbox_inches='tight')
    plt.close(fig4)
    ...
    fig6.savefig('plots/temporal_trends.jpg', bbox_inches='tight')
    plt.close(fig6)

    # Create and save trend analysis plot
    fig7 = plot_trend_analysis(data)
    fig7.savefig('plots/trend_analysis.jpg', bbox_inches='tight')
    plt.close(fig7)
